...
__declspec(dllexport)
int retryix_get_device_info(int device_index, RetryIXDeviceInfo* info) {
...
__declspec(dllexport)
int retryix_export_device_info_json(int device_index, char* json_buffer, int buffer_size) {
...
__declspec(dllexport)
int retryix_enumerate_platforms(char* buffer, int buffer_size) {
    cl_platform_id platforms[8];
    cl_uint num_platforms = 0;
    cl_int status = clGetPlatformIDs(8, platforms, &num_platforms);
    if (status != CL_SUCCESS || num_platforms == 0) return -1;
...
// 初始化邏輯
// return clGetPlatformIDs(max, platforms, num_found);
...
__declspec(dllexport) int retryix_enumerate_devices() {
